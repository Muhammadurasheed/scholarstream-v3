...
const Step6Background: React.FC<Step6Props> = ({ data, onNext, onSkip }) => {
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="text-center space-y-3">
        ...
        <p className="text-lg text-muted-foreground max-w-lg mx-auto">
          Many opportunities are specifically for students like you
        </p>
      </div>

      <Card className="max-w-2xl mx-auto p-4 bg-primary/5 border border-primary/20 shadow-sm">
        <div className="flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              This information helps us find targeted opportunities. It's kept completely private and secure.
            </p>
          </div>
        </div>
      </Card>
      ...
      {selected.length > 0 && !selected.includes('prefer-not') && (
        <div className="max-w-2xl mx-auto p-4 bg-green-500/10 border border-green-500/20 rounded-xl flex gap-3 items-start animate-fade-in">
          <Target className="h-5 w-5 text-green-600 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-foreground">
            Great! We'll show you <span className="font-semibold text-green-600">{selected.length * 15}+ specialized opportunities</span> designed for students with your background.
          </p>
        </div>
      )}
